...
<div class="mx-2 flex flex-col gap-4 md:flex-row">
  ...
  <div class="flex flex-col self-center md:self-auto">
    {#if player}
      <span class="overflow-ellipsis whitespace-nowrap text-center font-caption text-6xl text-white md:text-start">{player?.name}</span>
      <span class="text-center font-caption text-white md:text-start">Joined {joinedAt}</span>
    {:else}
      <div class="bg-slate-700 h-14 w-36 rounded my-1 animate-pulse"></div>
      <div class="bg-slate-700 h-6 w-36 rounded my-1 animate-pulse"></div>
    {/if}
    <div class="grow" />
